...
// for updating the training percentage
                ...
                if(result.status=='error') {
                    ...
                    for(let i in this.sorteddatalist){
                        if(this.sorteddatalist[i].slides!=null){
                           // console.log(this.ApiService.fileimgsslurl+this.sorteddatalist[i].slides[0].substr(3,this.sorteddatalist[i].slides[0].length));
                            this.sorteddatalist[i].firstslide=this.ApiService.fileimgsslurl+this.sorteddatalist[i].slides[0].substr(3,this.sorteddatalist[i].slides[0].length);
                        }
    ...
    getslide(val:any){
      if(this.curitem==null || this.curitem!=val){
          ...
          return this.ApiService.fileimgsslurl+this.sorteddatalist[val].slides[this.cureentcursor].substr(3,this.sorteddatalist[val].slides[this.cureentcursor].length);
      }
      else{
    ...
    markasdonetraninglesson(item,i){
        ...
        this._http.post(link, {source:'tranningcategory',data:data,sourceobj:objarr})
            .subscribe((res) => {
...
/*    getimageoffile(fileservername){
        ...
        const link = this.ApiService.nodesslurl+'getslidevalues';
        this._http.post(link,data)
            .subscribe(res=>{
                let result;
                result=res;
    ...
    showimag(img){
        ...
        return this.ApiService.fileimgsslurl+tempvar;
    }
