...
export class ListingComponent implements OnInit {
    ...
    constructor(public apiService: ApiService, public router: Router, public _http: HttpClient, public modal: BsModalService, formgroup: FormBuilder, private cookeiservice: CookieService,public sanitizer: DomSanitizer) {
    ...
    togglestatus(item: any) {
        ...
        console.log(item.status);
        const link = this.apiService.nodesslurl + 'togglestatus?token=' + this.cookeiservice.get('jwtToken');
        /* console.log('link');
         console.log(link);*/
    ...
    togglelockedstatus() {
        ...
        console.log(status);
        this.modalRef1.hide();
        const link = this.apiService.nodesslurl + 'togglelockedstatus?token=' + this.cookeiservice.get('jwtToken');
        /* console.log('link');
         console.log(link);*/
                    ...
                    for (let c in this.dataForm.controls) {
                        ...
                        for (let j in this.formdataval) {
                            ...
                            if (this.formdataval[j].name == c && this.formdataval[j].inputtype == 'image') {
                                ...
                                const link = this.apiService.base64encode + '&img=' + this.dataForm.controls[c].value + '&type=' + folder;
                                this._http.get(link)
                                    .subscribe(res => {
                                        let result: any;
                                        result = res;
    ...
    confirmdelete(template: TemplateRef<any>) {
        ...
        const link = this.apiService.nodesslurl + 'deletesingledata?token=' + this.cookeiservice.get('jwtToken');
        /* console.log('link');
         console.log(link);*/
    ...
    formsubmit() {
        ...
        if (this.dataForm.valid && this.submitval == 1) {
            const link = this.apiService.nodesslurl + 'addorupdatedata';
            console.log('link');
            console.log(link);
            console.log(this.router.url);
            console.log(this.formdataval);
            console.log(this.dataForm.value);
            this._http.post(link, { source: this.formsourceval.table, data: this.dataForm.value, sourceobj: this.formsourceval.objarr, token: this.cookeiservice.get('jwtToken') })
                .subscribe(res => {
                    let result: any;
                    result = res;
                    this.issubmit = 0;
    ...
    /*   onFileChanged(event,control:any,i:any){
     ...
     const link = this.apiService.uploadurl+'?imagefolder='+control.imagefolder;
     this._http.post(link,uploadData)
     .subscribe(event => {
     let res: any = {};
     res = event;
     if(res.error_code == 0){
     //  this.image = this.apiService.filepathurl+ res.filename;
    ...
    onUploadOutput(control: any, i, output: UploadOutput): void {
        ...
        if (output.type === 'allAddedToQueue') {
            const event: UploadInput = {
                type: 'uploadAll',
                url: this.apiService.nodesslurl + 'uploads',
                method: 'POST',
            };
    ...
    syncgoogle() {
        const link = this.apiService.nodesslurl + 'modifyusercalender';
        let data = {
    ...
    toggleFromSelect(event:any,item:any){
        ...
        console.log(status);
        const link = this.apiService.nodesslurl + 'addorupdatedata?token=' + this.cookeiservice.get('jwtToken');
        /* console.log('link');
         console.log(link);*/
         let data  = { 
